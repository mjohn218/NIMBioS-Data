...
disp('Stead State Period Average Beyond 300 s');
[epkm, epks] = findPeriod(EAvgHeight,time,300);
[ewlm, ewls] = findPeriod(EAvgWL, time,300);
[esymm, esyms] = findPeriod(ESymVal, time,300);
[etotm, etots] = findPeriod(ETotal, time,300);
[dpkm, dpks] = findPeriod(DAvgHeight,time,300);
[dwlm, dwls] = findPeriod(DAvgWL, time,300);
[dsymm, dsyms] = findPeriod(DSymVal, time,300);
[dtotm, dtots] = findPeriod(DTotal, time,300);
% Period Mean Calculations
disp('-----------------------------------------------------------');
