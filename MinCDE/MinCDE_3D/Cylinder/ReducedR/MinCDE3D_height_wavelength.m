...
for i = 1:length(time_redr)
   pksE = findpeaks(EminDT_redr(i,:),'MinPeakDistance',1);
   maxheightE(i,1) = mean(pksE);
   localminE = findpeaks(-EminDT_redr(i,:),'MinPeakDistance',1);
   if length(pksE) == 1 %&& length(localminE) == 0
       avglengthE(i,1) = 0;
    elseif length(pksE) == 1
        [lowpksE lowpksEloc] = findpeaks(EminDT_redr(i,:),'MinPeakDistance',1);
        avgE = 0;
