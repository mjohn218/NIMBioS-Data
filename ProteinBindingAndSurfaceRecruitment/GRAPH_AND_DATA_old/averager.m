...
mat = cnt/maxfol;
indices = find(diff(mat(:,1))==0);

mat(indices,:) = [];

ts = [0 logspace(-7,log10(mat(end,1)),100)];
As(:,3) = interp1(mat(:,1), mat(:,3), ts)';
As(:,4) = interp1(mat(:,1), mat(:,4), ts)';
As(:,5) = interp1(mat(:,1), mat(:,5), ts)';
As(:,6) = interp1(mat(:,1), mat(:,6), ts)';
As(:,7) = interp1(mat(:,1), mat(:,7), ts)';
As(:,13) = interp1(mat(:,1), mat(:,13), ts)';
As(:,10) = interp1(mat(:,1), mat(:,10), ts)';

sum = (As(:,5)+As(:,6)+As(:,7))/2;

semilogx(ts,[As(:,3) As(:,4) sum-As(:,7) sum-As(:,6) sum-As(:,5) As(:,13)+As(:,10)],'LineWidth',2);
...
hold on
load det3D_cube.mat
plot(det3D(:,1),det3D(:,2:end),'k--');

% dlmwrite('A.dat',[mat(:,1) mat(:,3)],'delimiter','\t','precision',6);
dlmwrite('A.dat',[ts(:) As(:)],'delimiter','\t','precision',6);
